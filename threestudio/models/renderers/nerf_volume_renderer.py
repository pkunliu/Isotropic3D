    ...
    def forward(
        ...
        **kwargs
        ...
        out = {
            ...
            "comp_rgba": torch.cat([comp_rgb, opacity],dim=-1).view(batch_size, height, width, -1),
        ...
        if self.training:
            ...
            if "normal" in geo_out:
                if self.cfg.return_comp_normal:
                    ...
                    comp_normal = (
                        (comp_normal + 1.0) / 2.0 * opacity + bg_color * (1.0 - opacity)
                    )  # for visualization
                    comp_normala = torch.cat([comp_normal, opacity], dim=-1)  # for visualization
                    out.update(
                        {
                            "comp_normal": comp_normal.view(
                                batch_size, height, width, 3
                            ),
                            "comp_normala": comp_normala.view(
            ...
            if "normal" in geo_out:
                ...
                comp_normal = F.normalize(comp_normal, dim=-1)
                comp_normal = (comp_normal + 1.0) / 2.0 * opacity + bg_color * (1.0 - opacity)# for visualization
                comp_normala = torch.cat([comp_normal, opacity], dim=-1)  # for visualization
                out.update(
                    {
                        "comp_normal": comp_normal.view(batch_size, height, width, 3),
                        "comp_normala": comp_normala.view(batch_size, height, width, 4),
                    }
                )
