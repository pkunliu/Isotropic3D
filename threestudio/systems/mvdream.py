...
class MVDreamSystem(BaseLift3DSystem):
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.save_image_grid(
            f"it{self.true_global_step}-test/{batch['index'][0]}_rgba.png",
            (
                [
                    {
                        "type": "rgba",
        ...
        self.save_image_grid(
            f"it{self.true_global_step}-test/{batch['index'][0]}_normala.png",
            (
                [
                    {
                        "type": "rgba",
                        "img": out["comp_normala"][0],
                        "kwargs": {"data_format": "HWC", "data_range": (0, 1)},
                    }
                ]
                if "comp_normal" in out
        ...
        self.save_image_grid(
            f"it{self.true_global_step}-test/{batch['index'][0]}_normal.png",
            (
                [
                    {
                        "type": "rgb",
    ...
    def on_test_epoch_end(self):
        ...
        self.save_img_sequence(
            f"it{self.true_global_step}-test-rgba",
            f"it{self.true_global_step}-test",
            "(\d+)\_rgba.png",
            save_format="gif",
            fps=30,
        ...
        self.save_img_sequence(
            f"it{self.true_global_step}-test-normala",
            f"it{self.true_global_step}-test",
            "(\d+)\_normala.png",
            save_format="gif",
            fps=30,
