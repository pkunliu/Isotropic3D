...
class SaverMixin:
    ...
    DEFAULT_RGBA_KWARGS = {"data_format": "HWC", "data_range": (0, 1), "rgba": True}
    DEFAULT_UV_KWARGS = {
        "data_format": "HWC",
        "data_range": (0, 1),
        "cmap": "checkerboard",
    ...
    def get_image_grid_(self, imgs, align):
        ...
        for col in imgs:
            ...
            if col["type"] == "rgb":
                ...
                rgba_kwargs = self.DEFAULT_RGBA_KWARGS.copy()
                rgba_kwargs.update(col["kwargs"])
                cols.append(self.get_rgb_image_(col["img"], **rgba_kwargs))
            elif col["type"] == "uv":
    ...
    def save_img_sequence(
        ...
        # imgs = [cv2.imread(os.path.join(img_dir, f), cv2.IMREAD_UNCHANGED) for f in imgs]

        if save_format == "gif":
            if rgba:
                # import pdb; pdb.set_trace()
                imgs[0].save(save_path, save_all=True, append_images=imgs[1:],duration=fps,loop=0,disposal=2)
                # imgs = [cv2.cvtColor(i, cv2.COLOR_BGRA2RGBA) for i in imgs]
            else:
